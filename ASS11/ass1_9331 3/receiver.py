...
#second handshake
        ...
        receiverLog.write("snd  %.3f SA %d 0 %d\n" %((time.clock()-timer_start)*1000,header[0], header[1]))


#third handshake
        datagram, address = server_socket.recvfrom(4096)
        header, data = extr_datagram(datagram.decode("utf-8"))
        receiverLog.write("rcv  %.3f A %d 0 %d\n" %((time.clock()-timer_start)*1000,header[0], header[1]))
...
def receive(socket):
    ...
    header, data = extr_datagram(datagram.decode("utf-8"))
    receiverLog.write("rcv  %.3f D %d 0 %d\n" %((time.clock()-timer_start),header[0], header[1]))
                ...
                while buffer and tof:
                    ...
                    #                print(part_of_message)
                        if part_of_message:
                            ...
                            socket.sendto(datagram.encode("utf-8"), address)
                            nbytes = len(part_of_message)
        

                            #ack_no+=mss
                            receiverLog.write("snd  %.3f D %d %d %d\n" %((time.clock()-timer_start),header[0],nbytes,header[1]))
...
#first fin receive
        ...
        print(header)
        print("first receive successed!")
        receiverLog.write("rcv  %.3f F %d 0 %d\n" %((time.clock()-timer_start),header[0],header[1]))
        ...
        socket.sendto(datagram.encode("utf-8"), address)
        print("second send successed!",header)
        receiverLog.write("snd  %.3f A %d 0 %d\n" %((time.clock()-timer_start),header[0],header[1]))
...
#third fin
        ...
        print("3000finnnnnnnnnnnnn",header)
        
        datagram = make_datagram(header, "")
        socket.sendto(datagram.encode("utf-8"), address)
        receiverLog.write("snd  %.3f FA %d 0 %d\n" %((time.clock()-timer_start),header[0],header[1]))
...
#forth fin
        ...
        print("4000finnnnnnnnnnnn",header)
        file.close()
        socket.close()
        sys.exit()
